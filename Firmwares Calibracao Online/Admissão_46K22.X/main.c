 ...
 * Admissão envia valor do angulo da borboleta pra o PIC GERENCIAMENTO
...
//                SSP1BUF = 0xAA;
                ...
                else
                {
                    ...
                    //ref_vbReq = 255 - (unsigned char)(ValorTotal/SPI_MAX); // tira a média dos valores recebidos da SPI
...
void controle_vb()
{
  ...
  // if (ref_vbReq > 223)                             ref_vbReq = 223;        // Impede fechamento total da vb
   ...
   pwm_10bits = (unsigned int)pwm * 4;      //Conversão 8bit´s para 10bit´s
...
void Control_up()
{
    ...
    Erro1 = y * 1.0 - Angle * 1.0; // Erro em relação ao set-point
...
void Control_down()
{
    Erro2 = y * 1.0 - Angle * 1.0; // Erro em relação ao set-point
    // Erro2 = SetPoint * 1.0 - Angle * 1.0; // Erro em relação ao set-point
    // Erro2 = y * 1.0 - Angle * 1.0; // Erro em relação ao set-point
    // Erro2 = 3 * 1.0 - Angle * 1.0; // Erro em relação ao set-point
...
void Configure_HW(void)
{
    ...
    RCONbits.IPEN = 1; //interrupão com prioridades
    
    Configure_PWM2();
    Configure_PWM3();
    Config_Timer0();
    Config_Timer2();
    Config_Timer5();        //Interrupção no pino RC2 para autorizar leitura I2c
}
...
void main(void)
{
    ...
    while (1)
    {
        if (!PORT_L15) // DESLIGA  o controle se não estiver com a linha 15 para não dar wind-up
        {
            Set_PWM2_Duty(0);
            Set_PWM3_Duty(0);
            EnableControl = 0;
            ...
            angle_aq[0] = (unsigned int)(Angle*10) >> 8;
            angle_aq[1] = (unsigned int)(Angle*10) & 0x00FF;
        }
        else
        {
            Enable_Interrupt(nINT_TIMER0);
            ...
            if (EnableControl)
            {
                ...
                if (Borboleta_sobe == 1) // Se a borboleta estiver subindo filtra a atuação
                {
                    filtro_vb();
                }
                else // senão o setpoint é direto
                {
                    y = SetPoint;
                }
                // pwm= Out2*10.23;
