...
{
  //SSP2IP = 1;
      SSP2CON1 = 0b00110001;              //habilita modo SPI // FOSC/4 //
  SSP2STAT = 0X00;                    //pega amostras no fim do bit e a trasmissão será na borda de descida
  TRISD0 = 0;
  TRISD1 = 1;
  //TRISD3 = 0;
  TRISD4 = 0;
...
void MASTER_SPI_WRITE1(unsigned char dados)
{
    ...
    //__delay_us(10);                    //espera slave processar informação        //COMENTADO PELO MARCUS E WELLINGTON (PRECISA VOLTAR?)
...
{
  SSP1CON1 = 0X24;                   //habilita modo SPI // habilita pino SS(slave select) pois é mais usual assim
  SSP1STAT = 0X40;                   //trasmissão será na borda de descida
  TRISA5 = 1;               //A5 SS in
...
void MASTER_SPI_WRITE2(unsigned char dados)
{
    ...
    __delay_us(10);                    //espera slave processar informação
