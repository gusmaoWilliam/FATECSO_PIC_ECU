...
unsigned char Rot1, Rot2, Pedal, MAP, TempAr, TempAg, angle1, angle2, battery; //buffer 12 em diante para dados serem mostrados no pc
...
/********CALIBRAÇAO ONLINE DECLARA CALIBRAÇAO ONLINE**********/

unsigned char flagCal = 0, flagCalAux = 0;
unsigned char bufSerial[10];
unsigned char bufCal[2];

/********CALIBRAÇAO ONLINE FIM CALIBRAÇAO ONLINE**********/
...
void interrupt isr(void)//vetor de interrupção
{
    ...
    {      
            ...
            else
            {
                ...
                if(buffer[ENDERECO_MASTER] == ENDERECO_GERENCIAMENTO)
                {
                    ...
                    if(index > 20 && flagCal == 0)
                    {
                        ...
                        TempAr = buffer_already[17] = buffer[18];
                        lambda = buffer_already[18] = buffer[19];
                        battery = buffer_already[19] = buffer[20];
                        index = 0;
        ...
        {  
            ...
            else if(flag_send_ger == 1 && flagCal == 1)
            {
                unsigned char buffer_test[15] = {0xBB, 0XFC, 0x11, 0x12, 0x13, 0x11, 0x12, 0x13, 0x11, 0x12, 0x13, 0x11, 0x12, 0x13, 0xBc};
                buffer_test[0] = bufCal[0];
                buffer_test[1] = bufCal[1];
                index = 0;
                SSP1BUF = buffer_test[index_read];
                
                index_read++;
                if(index_read >=15)  //Mestre esta lendo dado 16 e 17 (angle1 e angle2)
                {
                    index_read = 0;
                    flagCal = 0;
                    flagCalAux = 0;
    ...
    if(RCIF == 1) //Teste se houve interrupção pelo RX (UART))
    {
       ...
       // char_serial_ant=char_serial;
        ...
        bufSerial[flag_serial] = char_serial;
        flag_serial ++;
...
//    Enable_Interrupt(nSPI1);
   ...
   // hwConfigurarLcd();
    ...
    while (1)
    {
        ...
        if(flag_serial >= 2)
        {
            if (bufSerial[0] == 'R') // Se o caracter recebido anteriormente é 'R', ele entra para aquisitar os dados
            {
                switch(bufSerial[1])     //testa o caracter recebido agora
                {
                     ...
                     case '2': // "R2" - Manda todos os parametros
                         ...
                         temp_inj = (TmrInj1 << 8) | TmrInj2;
                         
                         Putc_TX1_16bits(rotacao); // 
                         Putc_TX1_16bits((unsigned int)map); //3  //valvula
                         Putc_TX1_16bits(angle); //2
                         Putc_TX1(TempAr);  //6 TEMPERATURA DO AR
                         Putc_TX1(TempAg);  //8
                         Putc_TX1(lambda);  //LAMBDA
                         Putc_TX1_16bits(temp_inj);     //12
                         Putc_TX1(IgDnt);  //LAMBDA
                         Putc_TX1(IgTrigger);
                         break;
            ...
            else if (bufSerial[0] == 'E') // Se o caracter recebido anteriormente é 'E', ele entra para enviar dados
            {
//                flagCalAux = 1;
//                while(1);
                switch(bufSerial[1])
                {
                    case '1':
                        ...
                        {
                            flagCalAux = 1;
                            bufCal[0] = bufSerial[2];
                            bufCal[1] = bufSerial[3];
                            flag_serial = 0;
                        }
                        break;
                    default:
...
//bit final_i2c_sincronismo = 0, final_i2c_admissao = 0, final_i2c_gerenciamento = 0;
//
//volatile unsigned char flagCal = 0, flagCalAux = 0;
//
///***************************************** Interrupts *******************************************/
//void interrupt isr(void)//vetor de interrupção
//{
