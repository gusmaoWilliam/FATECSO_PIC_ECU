...
{
    PIE1bits.TMR1IE = 0; // Desabilita a interrupção de TIMER1 por overflow
    /*************************************************
     *T1CON PS                                         *
     ...
     **************************************************/
    T1CONbits.T1CKPS1 = 1; // prescaler de 1:4
    T1CONbits.T1CKPS0 = 0; // prescaler de 1:4
    /*************************************************/
    T1CONbits.T1SOSCEN = 0; // Circuito secundário de oscilação desabilitado
    T1CONbits.TMR1CS = 0; // fonte de clock é o ciclo de máquina
    T1CONbits.T1RD16 = 1; // operando com 16 bits


    T1CONbits.TMR1ON = nENABLE; //habilita o TIMER1 para começar a contagem

    INTCONbits.RBIE = 1; // habilita a interrupção por mudança de estado no PORTB
}
...
void Config_Timer3(void)
{
    ...
    T3CONbits.TMR3CS = 0; // instrução por ciclo de máquina
    T3CONbits.T3RD16 = 1; // opera com 16 bits
    TMR3L = 0;
    TMR3H = 0;
...
void Config_Timer0(void)
{
    ...
    T0CONbits.T08BIT = 0; // configurado como 16bits
