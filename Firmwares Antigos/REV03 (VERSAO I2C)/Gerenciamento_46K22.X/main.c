 ...
 * Versão 4 do gerenciamento FLEX, recalibração da lógica de distúrbios de carga acoplados ao motor
 *
 * PERALTA
 * Versão com implementação do controle lambda no PIC gerenciamento , trocando o lugar de envio do tempo de injeção
 * (envia por primeiro)
 * Troca de lugar o envio do estado de controle de rotação de marcha lenta.
 * Sem sincronismo na comunicação entre os PICs Gerenciamento e Sincronismo
 *
 *
 * PERALTA , ENDRYL, WELLINGTON.
 * Versão com protótipo do controle lambda no uc de gerenciamento (funcionamento intermitente).
 *  Correção do tempo de injeção do controle lambda feita antes de todas as correções
 *  Última versão sem alterações na comunicação
 *
 * PERALTA
 *O gerenciamento agora recebe os valores de angulo da borboleta do admissão(thales) , por enquanto em oito bits para teste
...
#define     MAX_TORQUE_1          35 // era 30 limitação da porcentagem de torque requerida
...
#define     Vazao_InjGOL         300.3// valor calculado para o POLO de Sorocaba//400.0      /*GOL vazão de 2,5 g/seg*/
...
#define     MIN_REFVB           30.0 // estava em 34 por isso arotação não caía
/*****************************************************************************************************/
#define     nPin_HIGH            1
#define     nPin_LOW             0
#define     nYes                 1
...
unsigned int marcus_so_fala_bosta = 1512;
...
unsigned char p_rail = 0; // variavel para contador de tempo da bomba ligada após a linha 15
...
/* VARIÁVEIS REFERENTES AO CONTROLE LAMBDA (EQUIPE FATEC SOROCABA)*/
float KPlambda = 0.07; // Kp fixo, variação pequena
...
int estado_controle_ml = 0; // variável que define o estado do controle de avanço de ignição
float t_ar; // temperatura do ar
int t_ar_com;
float m_ar; //Rotina ECU Freescale
float m_comb, delta_estequiometria, corr_mc_ma;
unsigned int porcentagem_alcool=83; //Rotina ECU Freescale
...
void interrupt high_isr(void)
{
    __EXT_INTERRUPT0__{
        ...
        // interrupção externa RB0, para detectar falha TIMER 1 está pegando a borda de descida( verificar no polo qual borda é)
        Disable_ExtInt(nExt_Int0); //  disabilita a interrupção externa
        Clear_ExtInt_Flag(nFLAG_ExtInt0); //  limpa flag de interrupção
        ...
        if ((t_periodo > t_periodo_ref) && (dente > 50)) // compara os períodos, atual com anterior para detecção de falha
        {
            /*------------------ Nova Volta -------------------------- */
            ...
            a = b = c = d = 0; // flags para debug no proteus  para ver se o software está chegando aqui , não serve pra nada na atuação
            /*------------------ Fim Nova Volta ----------------------- */
        }

        t_periodo = t_periodo << 1; // multiplica por dois   para armazenar o valor do período da falha
        t_periodo_ref = t_periodo; // atualiza o valor do período  de referencia da falha

        conta_dente();

        Set_Timer1(0); // Inicia medicao do t_low
        ClearFlag_Interrupt(nFLAG_TMR1);
        Enable_Interrupt(nINT_TIMER1); // habilita a interrupção do timer 1
    ...
    __Interrupt_TMR3__ // interrupção que calcula todos os valores do gerenciamento
    {
        Disable_Interrupt(nINT_TIMER3); // desabilita interrupção de timer3 para que não estoure uma interrupção dentro da outra
        ClearFlag_Interrupt(nFLAG_TMR3); // limpa flag de interrupção
        ...
        if (volta_i2c) // estado 1 da maquina: faz as leituras
        {
            ...
            if (Send_i2c)
            {
                ...
                // outros valores para o comunicação
                ...
                buffer_i2c[15] = angle >> 8;     //...
                buffer_i2c[16] = angle & 0x00FF; //16 bits
            ...
            if (rotacao > 700)
            {
                ...
                m_comb = m_ar / (12.7 - delta_estequiometria); // estequiometria E25//13.3;//12.7 Rotina ECU Freescale    em mg!!!    12.7 é a estequiometria Fluxo:176.93g/min
                m_comb = m_comb *corr_mc_ma; // 1.25; // correção da massa de combustível
                
               if (chave_geral_control_lambda == 1) // chave controle geral desabilita o controle lambda caso haja alguma presença de distúrbio de carga ao motor para não causar alterações na rotação
                {
                    if (flag == 1) // flag condiciona que o contrle lambda só seja chamado caso a temperatura de operação da sonda seja atinjida
                    {
                        control_lambda();
                    }
                }
                ...
                //tempo_inj_temp = tempo_inj_temp; // alteração bruninho, quando estavamos na fatec para estabilizar ml
                ...
                // ALTERAÇÃO PERALTA PARA TESTE DE  injeçao
            ...
            else
            {
                ...
                //ref_vb = 40.0; // Abertura Inicial p/ o motor pegar (pré partida)
            }
            // nPin_TESTE = 0;
        }
        else //Volta_i2c// maquina de estado dois
        {
            ...
            if (pressao_ar < 30.0) address_2 = 0; // se a pressão do ar for menor do que 30  o índice é zerado(altas rotações)
            else if (pressao_ar > 120.0) address_2 = 9; // se for maior do que 120 o índice vai para o valor máximo(baixas rotações))
            else address_2 = (unsigned char) ((pressao_ar / 10) - 3); //caso não esteja nos extremos ele faz a conta para achar o índice
            ...
            n = tempo_bob_temp / periodo; // faz o cálculo para saber quantos dentes cabem no período de acionamento da bobina
            //ig_14_temp = ig_refGOL[address][address_2] - n; // aqui ele faz o cálculo para poder acionar a bobina de modo que
            ...
            //            /*degrau no avanço de ignição para identificação do modelo do motor*/
            ...
            if ((ig_dente_14_temp == 59) || (ig_dente_14_temp >= 60)) ig_dente_14_temp = 58; // Para o caso em que o sinal sobe na falha pq ele não consegue contar o dente 59
            ...
            // subtrai a variável que contém a parte inteira da váriável q contém a parte decimal do dente, acha a parte decimal
            // do dente de maneira isolada, multiplica pelo período para saber quanto tempo ele vai ter q contar para passar a parte decimal
            ...
            d = 1;
            Send_i2c = 1; // seta flag do envio da spi para na próxima volta calcular
            //nPin_TESTE = nPin_LOW;
        }
        b = 1;
        volta_i2c = !volta_i2c;
...
void interrupt low_priority Low_Isr(void)
{
    __Interrupt_TMR1__{
        ...
        else if (p_rail <= 100) // conta toda vez que entra na interrupção até aproximadamente 2s
        {
        ...
        {
            nRELE_BOMB_INJ = nPin_LOW; // desliga a bomba após 2s
        }
        ...
        //ref_vb = pedal_ad - 3; // relaciona a posiçao do pedal solto com o valor mínimo que vc quer na borboleta
        ref_vb = 36;
        ...
        if (dm == 0)
        {
            ...
            if (pedal_ad <= nPEDAL_MINGOL + 2)// pedal solto
            {
                ...
                else if (estado_temp_ml == 0 && cont_partida < 2500 && rotacao > 5)
                {
                    ...
                    {
                    case 0: ref_vb = 37; // 37situação de partida 5,0 graus
                        break;

                    case 1: ref_vb = 37; // 37 disturbio de carga acoplada ao motor borboleta em 5,5 graus
                        kp_inj = 1.4; // ganho de injeção para correção da rotação
                        break;

                    case 2: ref_vb = 35; // "mini" dash poat  para o motor não morrer na volta para a lenta borboleta em 4 graus
                        break;
                    }
...
//                    nLED_TEST = nPin_LOW;
                ...
                if (estado_temp_ml == 1) // entra só na primeira vez da marcha lenta para escolher o estado (tambem depois que acelera))
                {
...
//                        nLED_TEST = nPin_HIGH;
                ...
                if (t_agua < 40 && estado_temp_ml == 2)
                {
                    ...
                    if ((t_agua >= 40 && t_agua < 50) && estado_temp_ml == 3)
                {
//                    nLED_TEST = nPin_LOW;
                    ref_vb = 35; //34; // ref vb é 3,5 graus
            ...
            else
            {
                ...
                ref_torque = ((pedal_ad - 39)*100) / (nPEDAL_MAXGOL - 39); // 39 é o mínimo valor do pedal mais 2
...
inline void conta_dente()
{
    ...
    if (dente == 3)
    {
        ...
        periodo = Get_Timer0(); // pega o período de um dente
        Set_Timer3(65036);
...
void map_ad_Read()
{
    ...
    // pressao_ar_10bits = ((map << 2) | (ADRESL >> 6))*0.115 + 5.5; // CALCULO DA PRESSÃO COM BASE NA CURVA DO POLO SOROCABA
...
inline void controle_rpm()
{
    ...
    /*if ((ref_vb_temp < nTPS_1000) && (rotacao > 1500))       ref_vb_temp = nTPS_1000; // PARA O MOTOR NÃO CAIR MUITO RÁPIDO
...
void control_lambda()
{
    ...
    // * Tensão da lambda menor que ~2,5V (confirma lambda aquecida)
...
void main(void)
{
    /* O sistema executará a funçao main apenas uma vez*/
//    nLED_TEST = nPin_HIGH; // acende um led no hardware para ver se o programa está chegando aqui
    Configure_HW();
...
//    nLED_TEST = nPin_HIGH;
    ...
    ig_dente_14 = 200; // seta um dente de referencia que não existe
    ij_dente_14 = 200; // seta um dente de referencia que não existe
    ig_tmr_disparo = 0;
    ij_tmr_disparo = 0;
    tempo_inj = 0;
    tempo_bob = 0;
    ...
    dente = 255; // valor inválido de dente
    rotacao = 0;
    falha_ok = 0;
    temp = 0;
    dm = 0;
    ...
    Enable_Interrupt(nINTERRUPT_GLOBAL); // Habilita chave de interrupção global
    Enable_Interrupt(nINTERRUPT_PERIPHERAL); // habilita a chave geral de interrupção dos periféricos
    Enable_Interrupt(nINT_TIMER1); // Habilita chave individual  de interrupção do timer1 POR OVERFLOW
    Enable_ExtInt(nExt_Int0); //  Habilita chave individual de interrupção externa
 ...
 *           Pedal1  RA1 <> : 03           PGM 38 : <> RB5 Sensor Pressão A/C
 ...
 *                   RC2 <> : 17               24 : <> RC5
 *                   RC3 <> : 18               23 : <> RC4
 *                   RD0 <> : 19               22 : <> RD3 Sensor de Combustível
